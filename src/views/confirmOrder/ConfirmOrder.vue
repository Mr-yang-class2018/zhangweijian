...
#confirmOrder {
  .scroll {
    ...
    overflow: hidden;
    .address {
      background: #fff url(../../assets/img/order1.png) -7px bottom repeat-x;
      ...
      <div v-if="isShow">
        <div class="address">
          ...
          <div v-else class="selectAddr" @click="$store.commit('ROUTERTO','/allAddr')">
            ...
            <p style="color:#666;font-size:14px;">{{address.takeover_addr}}</p>
          </div>
        </div>
        <div v-for="(item,index) in shop" :key="index" style="padding: 15px;">
          ...
          <div class="ordertail">
            <div class="listimg">
              <img
                :src="$store.state.urlPath+'/goods/'+item.img_cover"
                alt
                style="width: 86px;float: left;"
              />
            </div>
            <div class="cardet" style="flex:5">
              <div
                style="line-height:20px;color:#333;font-size:14px;text-align:left;"
              >{{item.goods_name}}</div>
              <div class="norm-box" style="font-size:14px;text-align:left;">
                <p class="norm">
                  <em
              ...
              <div class="pricenum">
                <span style="float:left;color:#f2270c;font-weight:bold;">￥{{item.money_now}}</span>
                <span style="float:right;color:#999;">x{{item.num}}</span>
              </div>
            </div>
          </div>
          <div style="text-align: left;float: left; width:100%;">
            <div style="height:36px;">
              <span>商品金额:</span> 
              <span style="float:right;">￥{{item.money_now}}</span>
            </div>
            <div style="height:36px;">
             <span>运费</span>
             <small style="font-size:12px;color:#999;">（总重：9.480kg）</small>
             <span style="color:#ff4142;float:right;">免运费</span>
            </div>
            <div style="height:36px;">
              <span>实付金额</span> 
              <span style="color:#ff4142;float:right;">￥{{item.money_now}}</span>
            </div>
          </div>
        </div>
